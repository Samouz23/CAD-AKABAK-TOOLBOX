// =======================================================
// FICHIER :  src/popup/basicCalculators.js
// RÔLE    :  Gestion du popup intermédiaire de sélection de calculateur physics basic
//            Permet de choisir quel calculateur ouvrir en popup
// =======================================================

/**
 * Affiche le dialogue de sélection de calculateur physics basic
 */
export function showBasicCalculatorSelector() {
  // Supprime les popups existants
  removeBasicSelectorPopup();

  const overlay = document.createElement('div');
  overlay.id = 'basic-selector-overlay';
  ...
  const calculators = [
    { id: 'ohm', label: 'P = U²/R Converter' },
    { id: 'parallel', label: 'Parallel Resistors' },
    { id: 'wavelength', label: 'Frequency & Wavelength' }
  ];
  ...
  const calculatorOptions = calculators.map(calc => `
    <label class="calc-option" data-calc-id="${calc.id}" style="
      ...
      transition: all 0.2s;
    ">
      <input type="radio" name="basic-calc" value="${calc.id}" style="
        cursor: pointer;
        width: 18px;
        height: 18px;
  ...
  toast.innerHTML = `
    ...
    <div style="padding: 20px 24px; display: flex; flex-direction: column; gap: 12px;">
      ${calculatorOptions}
      <div id="basic-selector-error" style="color: rgb(239, 68, 68); font-size: 13px; font-weight: 500; display: none; margin-top: 8px;"></div>
    </div>
  ...
  closeButtons.forEach(btn => btn.addEventListener('click', removeBasicSelectorPopup));

  overlay.addEventListener('click', (e) => {
    if (e.target === overlay) removeBasicSelectorPopup();
  });

  const openBtn = toast.querySelector('#open-calculator-btn');
  const errorLabel = toast.querySelector('#basic-selector-error');
  ...
  if (openBtn) {
    openBtn.addEventListener('click', () => {
      ...
      const selectedCalc = toast.querySelector('input[name="basic-calc"]:checked');
      if (!selectedCalc) {
      ...
      window.electronAPI.openToolInNewWindow({ 
        toolName: `basic-${selectedCalc.value}`, 
...
function removeBasicSelectorPopup() {
  const overlay = document.getElementById('basic-selector-overlay');
  if (overlay) {
    overlay.remove();
  }
...
if (!document.querySelector('#basic-popup-animations')) {
  const style = document.createElement('style');
  style.id = 'basic-popup-animations';
  style.textContent = `
    @keyframes fadeIn {
      from { opacity: 0; }
      to { opacity: 1; }
